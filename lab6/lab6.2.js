...
// Define the scales here so they can be reused across the chart updates
...
var ascending = true; // Flag to track sort order
...
xScale.domain(d3.range(dataset.length)); // Re-update xScale domain to fit sorted data

svg.selectAll("rect")
    .data(dataset)
    .transition() // Add a smooth transition for sorting
    .duration(500)
    .attr("x", function(d, i) { 
        return xScale(i); })// Update x position based on new sorted index
    .attr("y", function(d) { return yScale(d); }) // Update y position
    .attr("height", function(d) { return h - padding - yScale(d); }); // Update height
